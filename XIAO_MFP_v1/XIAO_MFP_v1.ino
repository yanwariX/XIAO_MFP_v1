...
// 負荷時静音設定
static const bool kEnableQuietLoadDroop = true;          // 負荷が高い時に速度指令を少し下げる
static const float kQuietDroopStartLoadPercent = 50.0f;  // 静音補正を始める負荷率
static const float kQuietDroopFullLoadPercent = 80.0f;   // 静音補正が最大になる負荷率
static const float kQuietDroopMaxReductionPercent = 50.0f;  // 最大で下げる速度指令率
static const bool kEnableCommandSlewLimit = false;        // 速度指令の急変を丸める
static const int kCommandSlewUnitsPerSecond = 4000;      // 1秒あたりの最大指令変化量
...
struct ServoRuntime {
  ...
  unsigned long lastCommandUpdateMs;
  float targetRpm;
  float measuredRpm;
...
static inline bool telemetryPollingEnabled() {
  return kEnableFeedbackTelemetry || kEnableBrowserMonitor || kEnableQuietLoadDroop;
}
...
static int applyQuietLoadDroop(uint8_t servoIndex, int speedUnits) {
  if (!kEnableQuietLoadDroop || servoIndex >= 2 || speedUnits == 0) {
    return speedUnits;
  }

  const ServoRuntime &runtime = g_servoRuntime[servoIndex];
  if (!runtime.feedbackValid || runtime.loadPercent <= kQuietDroopStartLoadPercent) {
    return speedUnits;
  }

  const float droopRange =
      kQuietDroopFullLoadPercent - kQuietDroopStartLoadPercent;
  if (droopRange <= 0.0f || kQuietDroopMaxReductionPercent <= 0.0f) {
    return speedUnits;
  }

  const float loadRatio = clampFloat(
      (runtime.loadPercent - kQuietDroopStartLoadPercent) / droopRange,
      0.0f,
      1.0f);
  const float reductionRatio =
      clampFloat(kQuietDroopMaxReductionPercent, 0.0f, 90.0f) / 100.0f;
  const float scale = 1.0f - (loadRatio * reductionRatio);
  int adjusted = static_cast<int>((static_cast<float>(abs(speedUnits)) * scale) + 0.5f);

  if (adjusted > 0 && adjusted < kClosedLoopMinCommand) {
    adjusted = kClosedLoopMinCommand;
  }
  return signOfInt(speedUnits) * adjusted;
}

static int applyCommandSlewLimit(ServoRuntime &runtime, int desiredUnits, unsigned long now) {
  if (!kEnableCommandSlewLimit ||
      kCommandSlewUnitsPerSecond <= 0 ||
      runtime.lastCommandUnits == 32767 ||
      runtime.lastCommandUpdateMs == 0) {
    runtime.lastCommandUpdateMs = now;
    return desiredUnits;
  }

  const unsigned long elapsedMs = now - runtime.lastCommandUpdateMs;
  runtime.lastCommandUpdateMs = now;
  int maxStep = static_cast<int>(
      (static_cast<unsigned long>(kCommandSlewUnitsPerSecond) * elapsedMs) / 1000UL);
  if (maxStep < 1) {
    maxStep = 1;
  }

  const int delta = desiredUnits - runtime.lastCommandUnits;
  if (abs(delta) <= maxStep) {
    return desiredUnits;
  }
  return runtime.lastCommandUnits + (signOfInt(delta) * maxStep);
}
...
static void sendServoSpeed(uint8_t servoIndex, int speedUnits, float targetRpm) {
  ...
  speedUnits = applyQuietLoadDroop(servoIndex, speedUnits);
  ...
  speedUnits = applyCommandSlewLimit(runtime, speedUnits, millis());
